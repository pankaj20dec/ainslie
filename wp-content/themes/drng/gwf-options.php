...
add_theme_page( __( 'GW Fonts', 'sampletheme' ), __( 'GW Fonts', 'sampletheme' ), 'edit_theme_options', 'theme_options', 'theme_options_do_page' );*/
add_theme_page( 'Google Web Fonts', 'GW Fonts Settings', 'edit_theme_options', 'google-fonts-settings', 'wp_gwf_fonts_settings' );
add_theme_page( 'Google Web Fonts', 'GW Fonts Update', 'edit_theme_options', 'google-fonts-update', 'wp_gwf_fonts_update' );
...
$un_headtype2 = get_option('_headline_font_2');
$bodytype = str_replace(' ' , '+', get_option('_body_font'));
$headtype = str_replace(' ' , '+', get_option('_headline_font'));
$headtype2 = str_replace(' ' , '+', get_option('_headline_font_2'));
...
if ($headtype2 != ""){
echo "<link href='" . $http . "://fonts.googleapis.com/css?family=" . $headtype2 . ":" . $font_var . "' rel='stylesheet' type='text/css'>";
...
button, input[type="submit"], h1, h2, h4, h5, h6, .entry-title, .section-heading, .services-section-content:after, .site-info, .see-more, .share-icons  { font-family: <?php echo get_option('_headline_font'); ?> !important; }

h3, .main-navigation a, .footer-menu li a, .press-post-content p, blockquote  { font-family:<?php echo get_option('_headline_font_2'); ?> !important; }
 <?php echo "</style>"; ?>
<?php } 
endif;
...
jQuery("select#<?php echo '_body_font'; ?>, select#<?php echo '_headline_font'; ?>, select#<?php echo '_headline_font_2'; ?>").change(function(){

var val = jQuery("select#<?php echo '_body_font'; ?>").val();

var val2 = jQuery("select#<?php echo '_headline_font'; ?>").val();
var val3 = jQuery("select#<?php echo '_headline_font_2'; ?>").val();
...
jQuery("#cFontStyleWColor13").text('#testtext<?php echo "_headline_font_2"; ?> { font-size: 16px; font-family: "'+ val3 +'" !important; }');
...
<style id="cFontStyleWColor11" type="text/css">
</style>
<style id="cFontStyleWColor12" type="text/css">
</style>
<style id="cFontStyleWColor13" type="text/css">
</style>
<?php
$http = (!empty($_SERVER['HTTPS'])) ? "https" : "http";
$un_bodytype = get_option('_body_font');
...
<?php endif; ?>
<?php if( get_option('_headline_font_2') ): ?>
#testtext<?php echo "_headline_font_2"; ?> {

font-size: 16px; font-family: <?php echo get_option('_headline_font_2'); ?>;
...
"name" => __("Headline Font", TEMPLATE_DOMAIN),
	"description" => __("Choose a font for the headline h1,h2,h4,h5,h6 text.", TEMPLATE_DOMAIN),
	"id" => "_headline_font",
	"type" => "select",
	"options" => $get_font_transiet,
	"default" => "Oswald"),
array(
"name" => __("Headline Font 2", TEMPLATE_DOMAIN),
	"description" => __("Choose a font for the Blog Paragraph.", TEMPLATE_DOMAIN),
	"id" => "_headline_font_2",
	"type" => "select",
	"options" => $get_font_transiet,
	"default" => "Oswald"),
...
?>
  <form id="wp-theme-options" method="post" action="" enctype="multipart/form-data">
    <table class="form-table">
      ...
      <tr class="<?php echo $value['hide_blk']; ?>" valign="top">
        ...
        <td><select name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>">
          ...
          <br />
          <label class="description" for="<?php echo $value['id']; ?>"><?php echo $value['description']; ?></label></td>
      </tr>
      <?php } ?>
      <?php } ?>
    </table>
    <div style="float: left; margin: 20px 40px 0 0;" class="submit">
      <input name="save" type="submit" class="button-primary sbutton" value="<?php echo esc_attr(__('Save Setting',TEMPLATE_DOMAIN)); ?>" />
...
echo '<div class="updated fade"><p><strong>'.  __('You need to update or save the google font list first.', TEMPLATE_DOMAIN) . '</strong></p></div>'; ?>
    <?php else: ?>
    <table class="form-table">
      <h3><?php echo count($get_font_transiet); ?>
        <?php _e('available Google Web Fonts', TEMPLATE_DOMAIN); ?>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<small><a href="#google-font-dev-api">Update the font family below</a></small></h3>
      <tr valign="top">
        <td><?php
...
foreach($get_font_transiet as $font): ?>
    ...
    <div style="float: left; margin: 40px 20px 0 0;" class="submit">
      <label><strong>
        <?php _e('Enter Your Web Fonts Developer API', TEMPLATE_DOMAIN); ?>
        </strong></label>
      &nbsp;&nbsp;
      ...
      &nbsp;&nbsp;&nbsp;
      <label><strong>
        <?php _e('Sort by', TEMPLATE_DOMAIN); ?>
        </strong></label>
      &nbsp;&nbsp;
      <select id="google-font-sort" name="google-font-sort">
    ...
    <div style="float: left; margin: 40px 10px 0 0;" class="submit">
      <input name="save" type="submit" class="button-primary sbutton" value="<?php echo esc_attr(__('Update Fonts List',TEMPLATE_DOMAIN)); ?>" />
      <input type="hidden" name="action" value="save" />
    </div>
    <div style="color:#666;width:100%; float: left; margin: 0;" > <small>Web Fonts Developer API Example: AIzaSyBmPRa5TGlBRbUUV-pVPU3GxXRkD4lBtUU<br />
      <br />
      ...
      trending: Sort the list by families seeing growth in usage (family seeing the most growth first) </small> </div>
  </form>
</div>
<?php } ?>
